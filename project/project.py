...
# image type
def img_estim(img, threshold = 50):
    # With kernel size
    blur = cv2.blur(img, (3, 3))
    if cv2.mean(blur)[0] > threshold:
        # The range for a pixel's value in grayscale is (0-50), 127 lies midway
        # (51 - 255) denotes light image
        return "light"
    else:
