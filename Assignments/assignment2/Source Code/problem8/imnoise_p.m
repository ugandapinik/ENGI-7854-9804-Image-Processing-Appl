...
%   $Revision: 5.20.4.6 $  $Date: 2008/03/17 22:11:34 $
  ...
  switch code
    case 'uniform' % Uniform noise
    b = a + p3 + ((p4 - p3)*randn(sizeA));
    
   case 'gaussian' % Gaussian white noise
    b = a + sqrt(p4)*randn(sizeA) + p3;
    
    case 'lognormal' % lognormal Noise
        b = a + p3 * exp(p4*randn(sizeA));
        
    case 'periodic' % lognormal Noise
        rows = size(a,1);
        cols = size(a,2);
        row_cos_elements = cos(linspace(0,8*pi,cols));
        row_sin_elements = sin(linspace(0,8*pi,cols));
        row_cos_elements = repmat(row_cos_elements,rows/2,1);
        row_sin_elements = repmat(row_sin_elements,rows/2,1);
        periodicn = zeros(sizeA);
        for k = 1:rows
            for l = 1:cols
                if mod(k,2) == 0
                    periodicn(k,l) = row_sin_elements(k/2,l);
                elseif mod(k,2) == 1
                    periodicn(k,l) = row_cos_elements((k+1)/2,l);
                end
            end
        end
        periodic = (p3*periodicn) + (p3 * (periodicn)');
        b = a + periodic;
        
  case 'erlang' %erlang noise
      rand_var = rand(sizeA);
      b = a + ((p3^p4)*(rand_var.^(p4-1))).*exp((-p3)*rand_var);
  ...
  case 'rayleigh' %Rayleigh Noise
      b = a + p3 + (-p4 * log(1-rand(sizeA)).^0.5); 
  case 'exponential' %exponential Noise
      b = a + ((-1/p3) * log(1 - rand(sizeA)));
...
if nargin > 1
  ...
  allStrings = {'gaussian', 'uniform', 'lognormal', 'salt & pepper', 'speckle',...
                'poisson','localvar','rayleigh','exponential','erlang','periodic'};
...
switch code
 case 'poisson'
  ...
  case 'uniform'
  p3 = 0;  % default minimum  
  p4 = 1;  % default max value
  ...
  if nargin > 3
    p4 = varargin{4};
    if p4 < p3
        eid = sprintf('Images:%s:invalidMaxValue',mfilename);
        msg = 'Maximum Value should be grater then minimum';
        error(eid,'%s',msg);
    end
    if ~isNonnegativeRealScalar(p4)
      eid = sprintf('Images:%s:invalidVariance',mfilename);      
      msg = 'For ''uniform'' noise, V must be a real nonnegative scalar.';
      error(eid,'%s',msg); 
    end
  end
  
  if nargin > 4
    eid = sprintf('Images:%s:tooManyUniformInputs',mfilename);      
    msg = 'Too many inputs for ''Uniform'' noise.';
    error(eid,'%s',msg); 
  end
 ...
 case 'gaussian'
  ...
  case 'lognormal'
  p3 = 0;     % default mean
  p4 = 0.25;  % default variance
  
  if nargin > 2
    p3 = varargin{3};
    if ~isRealScalar(p3)
      eid = sprintf('Images:%s:invalidMean',mfilename);
      msg = 'For ''lognormal'' noise, M must be a real scalar.';
      error(eid,'%s',msg);
    end
  end
  ...
  if nargin > 3
    ...
    if ~isNonnegativeRealScalar(p4)
      eid = sprintf('Images:%s:invalidVariance',mfilename);      
      msg = 'For ''lognormal'' noise, V must be a real nonnegative scalar.';
      error(eid,'%s',msg); 
    end
  end
  ...
  if nargin > 4
    eid = sprintf('Images:%s:tooManyGaussianInputs',mfilename);      
    msg = 'Too many inputs for ''lognormal'' noise.';
    error(eid,'%s',msg); 
  end
 ...
 case 'salt & pepper'
  ...
  if nargin > 2
   ...
   case 'periodic'
  p3 = 1;   % default density
  
  if nargin > 2
    p3 = varargin{3};
    if ~isNonnegativeRealScalar(p3) || (p3 > 1)
      eid = sprintf('Images:%s:invalidNoiseDensity',mfilename);            
      msg1 = 'For ''periodic'' noise, D must be a real nonnegative ';
      msg2 = 'scalar less than or equal to 1.';
      msg = sprintf('%s\n%s',msg1,msg2);
      error(eid,'%s',msg);
    end
    ...
    if nargin > 3
      eid = sprintf('Images:%s:tooManySaltAndPepperInputs',mfilename);      
      msg = 'Too many inputs for ''periodic'' noise.';
      error(eid,'%s',msg);
    end
  end
...
case 'exponential'
  ...
  if nargin > 2
    ...
    if ~isNonnegativeRealScalar(p3) || (p3 > 1)
      eid = sprintf('Images:%s:invalidNoiseDensity',mfilename);            
      msg1 = 'For ''exponential'' noise, D must be a real nonnegative ';
      msg2 = 'scalar less than or equal to 1.';
      msg = sprintf('%s\n%s',msg1,msg2);
      error(eid,'%s',msg);
    end
    ...
    if nargin > 3
      eid = sprintf('Images:%s:tooManySaltAndPepperInputs',mfilename);      
      msg = 'Too many inputs for ''exponential'' noise.';
      error(eid,'%s',msg);
    end
  end
  ...
  if nargin > 2
    ...
    if ~isRealScalar(p3)
      eid = sprintf('Images:%s:invalidMean',mfilename);
      msg = 'For ''erlang'' noise, M must be a real scalar.';
      error(eid,'%s',msg);
    end
  end
  ...
  if nargin > 3
    ...
    if ~isNonnegativeRealScalar(p4)
      eid = sprintf('Images:%s:invalidVariance',mfilename);      
      msg = 'For ''erlang'' noise, V must be a real nonnegative scalar.';
      error(eid,'%s',msg); 
    end
  end
  ...
  if nargin > 4
    eid = sprintf('Images:%s:tooManyGaussianInputs',mfilename);      
    msg = 'Too many inputs for ''erlang'' noise.';
    error(eid,'%s',msg); 
  end
 ...
 case 'localvar'
  ...
  elseif nargin == 4
    ...
    if nargin > 2
        ...
        if ~isRealScalar(p3)
            eid = sprintf('Images:%s:invalidMean',mfilename);
            msg = 'For ''rayleigh'' noise, M must be a real scalar.';
            error(eid,'%s',msg);
        end
    end
  ...
  if nargin > 3
    ...
    if ~isNonnegativeRealScalar(p4)
      eid = sprintf('Images:%s:invalidVariance',mfilename);      
      msg = 'For ''rayleigh'' noise, V must be a real nonnegative scalar.';
      error(eid,'%s',msg); 
    end
  end
  ...
  if nargin > 4
    eid = sprintf('Images:%s:tooManyGaussianInputs',mfilename);      
    msg = 'Too many inputs for ''rayleigh'' noise.';
    error(eid,'%s',msg); 
  end
