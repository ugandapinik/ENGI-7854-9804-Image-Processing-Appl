clc;
F=imread('puppy.jpg'); % read image
F=rgb2gray(F);
im_size=size(F);
p=2*im_size(1);Q=2*im_size(2); % size of image	
FTIm=fft2(double(F),p,Q);
D0= 0.1*im_size(1)% cut-off frequency 0.1 
n=0
Filter=lp_hp_filters('ideal','lp',p,Q,D0,n);
Filtered_image=real(ifft2(Filter.*FTIm));
Filtered_image=Filtered_image(1:im_size(1),1:im_size(2));
Fim=fftshift(FTIm); % fast fourier transform
FTI=log(1+abs(Fim));
Ff=fftshift(Filter);
FTF=log(1+abs(Ff));
